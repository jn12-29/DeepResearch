...
class Visit(BaseTool):
    ...
    def call_server(self, msgs, max_retries=2):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"[SummaryModel] attempt {attempt+1}/{max_retries} model={model_name} base={url_llm}")
                ...
                if content:
                    ...
                    print(f"[SummaryModel] OK content_len={len(content)}")
                    return content
                else:
                    print(f"[SummaryModel] WARNING empty response")
            except Exception as e:
                print(f"[SummaryModel] ERROR attempt {attempt+1}/{max_retries}: {type(e).__name__}: {e}")
    ...
    def jina_readpage(self, url: str) -> str:
        ...
        for attempt in range(max_retries):
            ...
            print(f"[Jina] attempt {attempt+1}/{max_retries} key={'set' if JINA_API_KEYS else 'EMPTY'} url={url[:80]}")
            try:
                response = requests.get(jina_url, headers=headers, timeout=timeout)
                if response.status_code == 200:
                    webpage_content = response.text
                    print(f"[Jina] OK content_len={len(webpage_content)}")
                    return webpage_content
                else:
                    print(f"[Jina] ERROR HTTP {response.status_code}: {response.text[:200]}")
                    raise ValueError("jina readpage error")
            except Exception as e:
                print(f"[Jina] EXCEPTION {type(e).__name__}: {e}")
                time.sleep(0.5)
                if attempt == max_retries - 1:
