#!/usr/bin/env python3
"""
DR latency / timing analysis: FP16 vs Int4-W4A16 on test_dr.
...
Output: analysis/dr_latency_analysis.md

Usage:
    python analysis/dr_latency_analysis.py [--outputs-dir path]
"""
...
def load_records(outputs_dir: Path, model_dir: str) -> list[dict]:
    """Load all records from iter1/2/3.jsonl for a model's test_dr dir."""
    records = []
    base = outputs_dir / model_dir / DR_DIR
    for it in ITERS:
...
def tool_breakdown_table(stats_map: dict[str, dict]) -> str:
    ...
    def hdr(title):
        h = (
            f"| {'工具':<{col_w}} | "
            + " | ".join(f"{'调用次数':>8}({'avg_s':>6})" for _ in labels)
            + " |"
        )
        return f"**{title}**\n\n" + h
...
def latency_dist_table(stats_map: dict[str, dict]) -> str:
    ...
    col_w = 20
    header = f"| {'区间 (s)':<{col_w}} |" + "".join(
        f" {l+' 占比%':>14} |" for l in labels
    )
...
def main():
    ...
    for label, model_dir in MODELS.items():
        ...
        print(f"[{label}] loaded {len(records)} records from {model_dir}/{DR_DIR}")
        stats_map[label] = extract_latency_stats(records)

    md_lines = [
        "# DR 时延分析：FP16 vs Int4-W4A16",
        "",
        f"> 数据目录：`inference/outputs/*/test_dr/`，合并 iter1/iter2/iter3",
        "",
        "## 1. 核心时延指标汇总",
        "",
